...
--> statement-breakpoint
CREATE TABLE `verifyEmails_table` (
	`id` int AUTO_INCREMENT NOT NULL,
	`user_id` int NOT NULL,
	...
	CONSTRAINT `verifyEmails_table_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
...
ALTER TABLE `verifyEmails_table` ADD CONSTRAINT `verifyEmails_table_user_id_users_table_id_fk` FOREIGN KEY (`user_id`) REFERENCES `users_table`(`id`) ON DELETE cascade ON UPDATE no action;